...
//        }
    ...
    private void selectCenterTabUI() {
        if (imageViewList == null || normalIconItems == null || titleItems == null) {
            return;
        }
        for (int i = 0; i < tabCount; i++) {
            switch (contentType) {
                case TabContentType.TYPE_NORMAL:
                    if (imageViewList.size() > i && normalIconItems.length > i) {
    ...
    private void selectNormalTabUI(int position, boolean showAnim) {
        if (tabList == null || imageViewList == null || textViewList == null || selectIconItems == null || titleItems == null) {
            return;
        }
        for (int i = 0; i < tabCount; i++) {
            if (i == position) {
                if (anim != null && showAnim && tabList.get(i) != null) {
                    YoYo.with(anim).duration(300).playOn(tabList.get(i));
                }
                switch (contentType) {
                    case TabContentType.TYPE_NORMAL:
                        if (imageViewList.size() > i && selectIconItems.length > i) {
